...
class URLShortener{
    function generateShortURL($n){
        ...
        $checkURL = $db->prepare('SELECT id FROM shortended_urls WHERE short_url = :new_url');
    ...
    function validate(){
        ...
        if(!empty($long_url)){
            ...
            $insertData = $db->prepare('INSERT INTO shortended_urls (long_url, short_url) VALUES (:long_url, :short_url)');
            $insertData->execute([
                'long_url' => $long_url,
                'short_url' => $shortURL
            ]);
